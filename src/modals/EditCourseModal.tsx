...
export default function EditCourseModal({ isOpen, setIsOpen, course, onSuccess }: EditCourseModalProps) {
    ...
    useEffect(() => {
        if (course) {
            ...
            setPreviewImage(course.image ? `http://localhost:7777${course.image}` : null);
        }
    }, [course, form]);
    ...
    const onSubmit = async (values: z.infer<typeof courseSchema>) => {
        try {
            ...
            await toast.promise(
                axiosInstance.patch(`/courses/${course.id}`, formData, {
                    headers: {
                ...
                }),
                {
                    ...
                    success: () => {
                        setIsOpen(false);
                        onSuccess?.();
                        return 'Курс успешно обновлен!';
                    },
                    error: (error) => {
                        console.error('Ошибка при обновлении курса:', error);
                        return error.response?.data?.message || 'Не удалось обновить курс';
                    },
                }
            );
        } finally {
    ...
    return (
        <Modal
            ...
            description="Внесите изменения в информацию о курсе"
        >
            <Form {...form}>
